...
const navLinks = [
  ...
  { href: '/search', label: '产品查询', labelEn: 'Product' },
    ...
    <header className="bg-[var(--color-primary)] text-white sticky top-0 z-50 shadow-md">
      <div className="max-w-7xl mx-auto px-4">
        <nav className="flex items-center justify-between h-16">
          ...
          <Link
            href="/search"
            className="hidden md:inline-flex items-center bg-[var(--color-accent)] hover:bg-[var(--color-accent-light)] text-white text-sm font-semibold px-5 py-2 rounded-lg transition-colors"
          >
            立即查询
          </Link>
        ...
        {open && (
          <div className="md:hidden border-t border-white/10 py-3 space-y-1">
            ...
            <Link
              href="/search"
              onClick={() => setOpen(false)}
              className="block mx-4 mt-2 text-center bg-[var(--color-accent)] hover:bg-[var(--color-accent-light)] text-white text-sm font-semibold px-5 py-2 rounded-lg transition-colors"
            >
              立即查询
            </Link>
          </div>
